...
// colours 
proj_data = config.col.map((x, i) => [x, proj_data[i]]);
// axis labels
...
points = svg
    ...
    .data(proj_data)
    .join("circle")
        .attr("cx", d => x_scale(d[1][0][0]))
        .attr("cy", d => y_scale(d[1][0][1]))
        .attr("r", 3) 
        .attr("fill",  d => d[0]);
    ...
    for (let i = 1; i < proj_mats.length; i++) {
        points = points
            ...
            .duration(1000/config.fps)
                .attr("cx", d => x_scale(d[1][i][0]))
                .attr("cy", d => y_scale(d[1][i][1]));
